...
1. Run `traj status`.
   Every group the sampler uses must have `stale` and `not_ok` at 0 for the current definitions.
...
5. Write `reports/<YYYY-MM-DD>-<topic>.md` with these sections.
   ...
   - **Proposed changes** to `features/*.yaml` and `samplers/*.yaml`.
6. When the user agrees, or asked for the changes up front, apply the proposed changes and run `traj validate`.
   Commit the report together with the definition changes, and name the report in the commit message.
